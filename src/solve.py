...
def solve_game_matrix(mat):
  """Calculate a mixed strategy solution for player A in a zero sum game.

  Args:
    mat - A two-dimensional numpy.array where each row represents the payouts
          for player A's pure strategies across all of player B's options.
  Returns:
    solution - A one-dimensional array where the i-th element represents the
          probability player A should assign to the strategy in row i.
    value - The expected payoff for the solution.
  """
...
def solve_for_player0_with_b1_known(mat, a0, b0, a1, b1):
  ...
  c = matrix([0.0 for i in range(b1 * a0 * b0)] + [1.0 for i in range(a0)])
  ...
  for a1i in range(a1):
    for b1i in range(b1):
      for ab0i in range(a0 * b0):
        G[b1i * a0 * b0 + ab0i][b1 * a0 * b0 + a1i * b1 + b1i] = mat[ab0i][
          a1i * b1 + b1i
        ]
  G = -matrix(G)
  ##  print "G rank: ", linalg.matrix_rank(array(G))
...
def solve_for_player0_with_b0_known(mat, a0, b0, a1, b1):
  ...
  c = matrix([1.0 for i in range(a0 * b0)] + [0.0 for i in range(b0 * a1)])
  ...
  G = [
    [0.0 for j in range(a0 * b0 + a1 + b0 * a1 * b1)] for i in range(a0 * b0 + b0 * a1)
  ]
  ...
  for a0i in range(a0):
    for b0i in range(b0):
      for ab1i in range(a1 * b1):
        G[a0i * b0 + b0i][a0 * b0 + a1 + b0i * a1 * b1 + ab1i] = mat[a0i * b0 + b0i][
          ab1i
        ]
