...
devastation_flights = [
  ["alexian", "eligor", "karin", "marmelee", "shekhtur"],
...
war_flights = [
  ["cadenza", "hikaru", "kallistar", "luc", "magdelina", "vanaah"],
...
def list_files(logdir, name=None, player_num=None):
  fulldir = "logs/" + logdir + "/"
  filenames = [fn for fn in os.listdir(fulldir) if os.path.isfile(fulldir + fn)]
  if name is None:
    return [fulldir + fn for fn in filenames]
  if player_num is None:
    return [fulldir + fn for fn in filenames if name in fn]
  return [fulldir + fn for fn in filenames if name == fn.split("_")[player_num]]
...
def victories(
  name, logdir="core_tests", return_timeouts=False, silent=False, per_name=False
):
...
def victory_csv(logdir="core_tests"):
  ...
  names_sums = sorted(zip(all_names, sums), key=itemgetter(1), reverse=True)
...
class BeatData(object):
  ...
  def __init__(self, name, opp_name, player_num, lines, replacement):
    ...
    line_starts = ["%s: " % n.capitalize() for n in (name, opp_name)]
    for i in (0, 1):
      my_lines = [line for line in lines if line.startswith(line_starts[i])]
      styles[i], bases[i], antes[i], induced_antes[i] = self.parse_line(
        my_lines[0], replacement
      )
      if len(my_lines) == 1:
        final_bases[i] = bases[i]
      else:
        final_bases[i] = self.parse_line(my_lines[-1], replacement)[1]
  ...
  def lines_starting_with(self, start):
    return [line for line in self.lines if line.startswith(start)]

  def lines_ending_with(self, end):
    end += "\n"
    return [line for line in self.lines if line.endswith(end)]
...
def entropy(logdir="core_tests"):
  ...
  for filename in list_files(logdir):
    ...
    with open(filename) as f:
      log = [line for line in f]
    starts = [i for i, line in enumerate(log) if line.startswith("Beat")]
    for start in starts:
  ...
  for title, d in list(dicts.items()):
    print(title)
    for name in d:
      d[name] = 2 ** sorted(d[name])[len(d[name]) / 2]
    items = sorted(list(d.items()), key=itemgetter(1))
    for i in items:
      print("%.1f %s" % (i[1], i[0]))
  my_names = [m[0] for m in sorted(list(my_entropy.items()), key=itemgetter(1))]
  opp_names = [m[0] for m in sorted(list(opp_entropy.items()), key=itemgetter(1))]
  diffs = []
  ...
  pylab.scatter(
    [x[1] for x in mys if x[0] in antes], [x[1] for x in opps if x[0] in antes], c="r"
  )
  pylab.scatter(
    [x[1] for x in mys if x[0] not in antes],
...
def parse_beats(name, logdir="core_tests"):
  ...
  for filename in list_files(logdir, name):
    ...
    file_beats = [
      BeatData(name, opp_name, number, log[starts[i] : starts[i + 1]], replacement)
...
def strategies(
  name, logdir="core_tests", beat=None, condition="", reverse_condition=False
):
  ...
  if condition:
    beats = [
      b for b in beats if bool(b.lines_starting_with(condition)) != reverse_condition
    ]
  if name == "adjenna":
...
def hitting(name, logdir="core_tests", printing=True):
  ...
  for beat in beats:
    ...
    for key, record in ((style, style_record), (base, base_record)):
  ...
  for record in style_record, base_record:
    total = HitRecord("All")
    for key in list(total.__dict__.keys()):
      if key != "name":
        total.__dict__[key] = sum([rec.__dict__[key] for rec in record.values()])
    all_recs = list(record.values()) + [total]
    ...
    for line in log:
      ...
      if style is None and line.startswith(name.capitalize() + ": "):
        for p in replacement:
          line = line.replace(p, replacement[p])
        style = line.split(" ")[1]
        if style not in iter(style_hits.keys()):
          style_hits[style] = 0
      ...
      if win or tie:
        styles = list(style_hits.keys())
        for style in styles:
          if style not in iter(record.keys()):
...
def specials(name=None, logdir="core_tests"):
  names = [name] if name else list(phrases.keys())
  stats = {"all": [0] * 16, "pulse": [0] * 16, "cancel": [0] * 16, "finisher": [0] * 16}
  games = 0
  for name in names:
    beats = parse_beats(name, logdir)
...
def anomalies(logdir="core_tests"):
  for name in all_names:
    strategy_dict = Counter(
      [(b.style, b.base, b.ante) for b in parse_beats(name, logdir)]
    )
...
def game_length(name, logdir="core_tests"):
  ...
  games = len([i for i in range(len(beats)) if beats[i].number == 1])
...
def pair_count(strat_dict, specials):
  styles = sorted(
    list(set([p[0] for p in list(strat_dict.keys()) if p[0] != "Special" or specials]))
  )
  bases = sorted(
    list(set([p[1] for p in list(strat_dict.keys()) if p[0] != "Special" or specials]))
  )
  antes = sorted(
    list(set([p[2] for p in list(strat_dict.keys()) if p[0] != "Special" or specials]))
  )
  count = array(
    [
      [
        sum(
          [
            strat_dict[(s, b, a)] if (s, b, a) in list(strat_dict.keys()) else 0
            for a in antes
          ]
        )
        for b in bases
...
def solution_percents(name, logdir="first_beats", beat=None):
    ...
    while True:
      ...
      if beat == None or current_beat in beat:
        ...
        while "%" in log[i]:
          ...
          if split[1] in list(style_dict.keys()):
            style_dict[split[1]] += percent
          else:
            style_dict[split[1]] = percent
          if split[2] in list(base_dict.keys()):
            base_dict[split[2]] += percent
          else:
            base_dict[split[2]] = percent
          i += 1
  bases = sorted(
    [b for b in list(base_dict.items()) if b[0] not in ["Cancel", "Pulse"]],
    key=itemgetter(1),
    reverse=True,
  )
  for b in bases:
  ...
  print("----------------")
  styles = sorted(
    [s for s in list(style_dict.items()) if s[0] != "Special"],
    key=itemgetter(1),
    reverse=True,
  )
  for s in styles:
...
def unbeatable_strategies(
  name, thresh=5, life_thresh=8, logdir="core_tests", outdir="logs/unbeatables/"
):
  ...
  for filename in list_files(logdir, name):
    ...
    while True:
      ...
      while i < len(log) and not log[i].startswith(
        "Unbeatable strategy for %s" % name.capitalize()
      ):
        i += 1
      if i == len(log):
        break
      ...
      if max(diffs) >= thresh:
        ...
        while True:
          j += 1
          if log[j].startswith("life: ") and not log[j - 2].startswith(
            name.capitalize()
          ):
        ...
        if life >= life_thresh:
          ...
          winfile.write("###############################\n")
          count += 1
    print(count, "vs.", (names[0] if names[0].lower() != name else names[1]))
    total += count
  winfile.close()
  print("total:", total)
...
def abarene_tokens(logdir="core_tests"):
  token_dict = {"Dizziness": 0, "Fatigue": 0, "Nausea": 0, "Lethargy": 0}
  beats = 0
  for filename in list_files(logdir, "abarene"):
    ...
    for line in log:
      if line.startswith("---"):
        ...
        beats += 1
      previous_line = line
  for token, count in list(token_dict.items()):
...
def adjenna_marker_beat(target=1, logdir="core_tests"):
  ...
  petrifications = len(
    [
      ...
      if beat.lines_ending_with("has 6 Petrificatoin Markers")
    ]
  )
  for beat in beats:
    lines = beat.lines_containing("Petrification markers on")
    beat.markers = int(lines[0][0])
  victories_with_target = sum(
    [beat.points for beat in final_beats if beat.markers >= target]
  )
...
def adjenna_marker_and_wins(target_beat, logdir="core_tests"):
  ...
  beats = parse_beats["adjenna", logdir]
  for i in range(len(beats)):
    if beats[i].beat_number == target_beat and beats[i - 1].beat_number < target_beat:
...
def alexian_tokens(logdir="core_tests"):
  ...
  beat_counts = [
    sum([beat.tokens for beat in beats if beats.number == i]) for i in range(16)
  ]
...
def alumis_styles(logdir="core_tests"):
  ...
  ghastly_power = [b for b in ghastly if b.lines_containing("Alumis gets +2 power")]
  ghastly_life = [
    b
    for b in ghastly
    if not all(
      line.startswith("Alumis loses 2 life")
...
def arec_tokens(logdir="core_tests"):
  token_dict = {"Fear": 0, "Hesitation": 0, "Mercy": 0, "Recklessness": 0}
  beats = 0
...
def eustace_counters(logdir="core_tests"):
  ...
  for b in range(1, 16):
    print("%d: %.1f" % (b, beat_tokens[b] / float(beat_counts[b])))
  print("Defrosting losses:", text("Eustace runs out of Frost", "eustace", logdir))
...
def hikaru_tokens(logdir="core_tests"):
  token_dict = {"Earth": 0, "Fire": 0, "Water": 0, "Wind": 0}
  beats = 0
...
def jager_signatures(logdir="core_tests"):
  ...
  for ante in full_count:
    if ante:
      print("%s: %d (%d)" % (ante, full_count[ante], refined_count[ante]))
  print("Total: %d (%d)" % (sum(full_count.values()), sum(refined_count.values())))


def iri_forms(logdir="core_tests"):
  beats = parse_beats("iri", logdir)
  beats = [b for b in beats if b.ante and b.base not in ("Pulse", "Dash")]
  forms = Counter(beat.ante for beat in beats)
  total = float(sum(forms.values()))
  print("Forms:")
  for form, count in list(forms.items()):
    print(form, count, percentify(count / total))
  for beat in beats:
    beat.current_form = beat.lines_ending_with(" Form")[0][:-1]
  transforms = Counter(
    beat.ante for beat in beats if beat.ante[1:-1] != beat.current_form
  )
...
def larimore_spending(logdir="core_tests"):
  ...
  for beat in beats:
    ...
    for i, line in enumerate(beat.lines):
      if line.startswith("Larimore spends "):
        ...
        if (
          beat.final_base == "Might"
          and beat.lines[i + 1] == "Larimore gets +%d power\n" % spend
        ):
  ...
  for style in styles:
    ...
    for spend in range(6):
      n = len([b for b in beats if b.style == style and b.style_spend == spend])
      print(spend, n)
      style_spend += spend * n
  ...
  might_spend = 0
  for spend in range(6):
    n = len([b for b in beats if b.final_base == "Might" and b.might_spend == spend])
    print(spend, n)
    might_spend += spend * n
...
def magdelina_spiritual(logdir="core_tests"):
  ...
  for beat in beats:
    if beat.style == "Spiritual" and "Level: 0\n" in beat.lines:
      for line in beat.lines:
        print(line)
...
def marmelee_counters(logdir="core_tests"):
  ...
  for beat in beats:
    ...
    for line in beat.lines:
      if (
        line[1:] == " Concentration counter\n"
        or line[1:] == " Concentration counters\n"
      ):
        beat.tokens = int(line[0])
        beat_tokens[beat.number] += beat.tokens
        break
...
def marmelee_spending(logdir="core_tests"):
  ...
  for filename in list_files(logdir, "marmelee"):
    ...
    for i in range(len(log)):
      ...
      if style is not None and line.startswith("Beat "):
        ...
        if key in list(style_spend_dict.keys()):
          style_spend_dict[key] += 1
        else:
          style_spend_dict[key] = 1
        spend = 0
      if re.search(r"Marmelee discards . Concentration Counters$", line):
        spend += int(line.split(" ")[2])
        if log[i + 1].startswith("Marmelee moves:"):
          spend += 0.1  # additional +.1 marks spending for move
      if line.startswith("Marmelee gains Soak"):
        spend += 0.01 * int(line[-1])
  ...
  for key in sorted(style_spend_dict.keys()):
    total = 0
    for otherkey in list(style_spend_dict.keys()):
      if otherkey[0] == key[0]:
        total += style_spend_dict[otherkey]
    print(key, style_spend_dict[key], percentify(style_spend_dict[key] / float(total)))
...
def oriana_tokens(logdir="core_tests"):
  ...
  for beat in beats:
    ...
    for line in beat.lines:
      if (
        line[1:] == " Magic Point token\n"
        or line[1:] == " Magic Point tokens\n"
        or line[2:] == " Magic Point token\n"
        or line[2:] == " Magic Point tokens\n"
      ):
        beat.tokens = int(line[0])
        beat_tokens[beat.number] += beat.tokens
        break
...
def runika_artifacts(logdir="core_tests"):
  ...
  activations = {
    a: text("%s is activated" % a, "runika", logdir) / games for a in artifacts
  }
  deactivations = {
    a: text("%s is de-activated" % a, "runika", logdir) / games for a in artifacts
  }
  removals = {a: text("%s is removed" % a, "runika", logdir) / games for a in artifacts}
  overcharges = {
    a: text("Runika overcharges her %s" % a, "runika", logdir) / games
    for a in artifacts
  }
  print("Activations:")
  for a in artifacts:
...
def xenitia_draw(logdir="core_tests"):
  ...
  for style, draws in list(style_dict.items()):
    print(style)
...
def parse_base_vs_induced_ante(name, logdir="core_tests"):
  ...
  strategy_dict = Counter([(beat.opp_base, beat.opp_induced_ante) for beat in beats])
  antes = sorted(list(set([s[1] for s in strategy_dict])))
  count = array([[strategy_dict.get((b, a), 0) for a in antes] for b in all_bases])
...
def post_processing_seth(beats):
  standard_antes = ["(Strike)", "(Shot)", "(Dash)", "(Burst)", "(Drive)", "(Grasp)", ""]
  for beat in beats:
