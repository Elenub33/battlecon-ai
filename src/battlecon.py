...
class Game:
  ...
  def __init__(
    ...
    self.log = []
    self.player = [
      character_dict[name0](self, 0, bases0),
      character_dict[name1](self, 1, bases1, is_user=interactive),
    ]
    for i in range(2):
  ...
  def log_unbeatable_strategies(self, log):
    ...
    for i in range(n):
      for j in range(m):
        ...
        s1 = self.strats[1][j]
        regular_0 = [
          k for k in range(len(s0)) if not isinstance(s0[k][0], SpecialAction)
        ]
        regular_1 = [
          k for k in range(len(s1)) if not isinstance(s1[k][0], SpecialAction)
        ]
  ...
  def simulate(self, s0, s1, state=None):
    ...
    # (up to pulse check)
      ...
      for p in self.player:
        if isinstance(p.base, Finisher) and p.base.devolves_into_cancel():
          p.base = p.cancel
    ...
    try:
      ...
      if state.stage <= 0:
        ...
        # priority tie
          ...
          if isinstance(self.player[0].base, Finisher) and isinstance(
            self.player[1].base, Finisher
          ):
            final_state = self.full_save(None)
            return self.CANCEL_BOTH_INDICATOR, final_state, self.fork_decisions[:]
          else:
  ...
  def execute_beat(self, post_clash=False):
    ...
    if value in self.CANCEL_INDICATORS:
      ...
      post_cancel_strats0 = [
        ...
        and s[0] != s0[0]
        and s[1] != s0[1]
        and not isinstance(s[0], SpecialAction)
      ]
      post_cancel_strats1 = [
        s
        for s in ss1
...
class Character(object):
  ...
  def __init__(self, the_game, n, base_set="alpha", is_user=False):
    ...
    self.name = re.sub(
      r"([a-z](?=[A-Z0-9])|[A-Z0-9](?=[A-Z0-9][a-z]))", r"\1 ", self.__class__.__name__
    )
    ...
    self.finisher_generators = set([self.bases[0], self.bases[1], self.bases[3]])
  ...
  def all_cards(self):
    return (
      ...
      + self.finishers
      + self.status_effects
      + [self.special_action, self.pulse, self.cancel, self.null_style, self.null_base]
    )
  ...
  def read_my_state(self, lines, board, addendum):
    ...
    if self.counter_name:
      ...
      next_line_index += 1
    if find_start(lines, "Special action available") or find_start(
      lines, "special action"
    ):
  ...
  def set_starting_setup(self, default_discards, use_special_actions):
    ...
    if self.game.first_beats:
      ...
      if self.is_user:
        ...
        b2 = [b for b in self.bases if b.alpha_name == b2.alpha_name][0]
  ...
  def discard_report(self, pile):
    ...
    other = [
      card.name
      for card in pile
      if not isinstance(card, Style) and not isinstance(card, Base)
    ]
  ...
  def get_pairs(self):
    unavailable_cards = self.discard[1] | self.discard[2]
    styles = sorted(set(self.styles) - unavailable_cards, key=attrgetter("order"))
    bases = sorted(set(self.bases) - unavailable_cards, key=attrgetter("order"))
  ...
  def input_pair(self):
    styles = sorted(list(set(m[0][0] for m in self.mix)), key=attrgetter("order"))
  ...
  def set_active_cards(self):
    self.active_cards = (
      [self.style, self.base] + self.get_active_tokens() + self.active_status_effects
    )
  ...
  def get_priority_pre_penalty(self):
    ...
    return (
      ...
      + sum(card.priority + card.get_priority_bonus() for card in self.active_cards)
    )
  ...
  def get_priority_bonuses(self):
    return (
      ...
      + sum(card.get_priority_bonus() for card in self.active_cards)
      + self.get_priority_bonus()
      + self.triggered_priority_bonus
  ...
  def get_printed_priority(self):
    return sum(
      card.priority for card in set((self.style, self.base)) & set(self.active_cards)
    )
  ...
  def get_printed_power(self):
    if self.alt_pair_power is None:
      return sum(
        card.power for card in set((self.style, self.base)) & set(self.active_cards)
      )
    else:
  ...
  def get_power_bonuses(self):
    return (
      sum(
        card.power for card in self.active_cards if card not in (self.style, self.base)
      )
      + sum(card.get_power_bonus() for card in self.active_cards)
  ...
  def get_minrange(self):
    ...
    if self.ignore_style_range(self) or self.opponent.ignore_style_range(self):
      cards = [c for c in cards if c is not self.style]
    minrange = (
      self.get_minrange_bonus()
      + sum(card.minrange + card.get_minrange_bonus() for card in cards)
      + self.opponent.give_minrange_penalty()
    )
    if self.opponent.blocks_minrange_bonuses():
      printed_minrange = sum(
        card.minrange for card in set((self.style, self.base)) & set(cards)
      )
  ...
  def get_maxrange_bonuses(self):
    return (
      ...
      + sum(card.get_maxrange_bonus() for card in self.active_cards)
  ...
  def get_maxrange(self):
    ...
    maxrange = (
      self.get_maxrange_bonus()
      + sum(card.maxrange + card.get_maxrange_bonus() for card in cards)
      + self.opponent.give_maxrange_penalty()
    )
    if self.opponent.blocks_maxrange_bonuses():
      printed_maxrange = sum(
        card.maxrange for card in set((self.style, self.base)) & set(cards)
      )
  ...
  def activate_card_triggers(self, trigger_name, params=[]):
    ...
    while ordered_cards:
      if len(ordered_cards) > 1:
        ...
        ans = self.game.make_fork(len(ordered_cards), self, prompt, options)
      else:
        ans = 0
  ...
  def can_hit(self):
    if self.standard_range():
      ...
      if self.game.debugging:
        self.game.report(
          "%s's range: %d-%d attack range: %d"
          % (self.name, self.get_minrange(), self.get_maxrange(), distance)
        )
      in_range = distance <= self.get_maxrange() and distance >= self.get_minrange()
      if self.game.debugging:
        self.game.report("in range: %s" % in_range)
        self.game.report(
          "all cards can hit: %s" % all(card.can_hit() for card in self.active_cards)
        )
    else:
      in_range = False
    return (in_range or self.special_range_hit()) and all(
      card.can_hit() for card in self.active_cards
    )

  def can_be_hit(self):
    if self.game.debugging:
      self.game.report("triggered dodge: %s" % self.triggered_dodge)
    return not self.triggered_dodge and all(
      card.can_be_hit() for card in self.active_cards
    )
  ...
  def cycle(self):
    ...
    if self.style is not self.special_action or (
      self.opponent.base is self.opponent.pulse and self.base is not self.pulse
    ):
  ...
  def take_damage(self, damage):
    ...
    self.life = max(self.life - final_damage, self.get_minimum_life())
    if self.game.reporting:
      self.game.report(
        self.name + " takes %d damage (now at %d life)" % (final_damage, self.life)
      )
    if self.life <= 0:
  ...
  def discard_token(self, token=None, verb="discard"):
    ...
    if token == None:
      ...
      # otherwise discard lowest value (with fake_fork for human)
        ...
        index = self.game.make_fork(len(self.pool), self, prompt, options, index)
      discarded = self.pool.pop(index)
      if self.game.reporting:
        self.game.report(self.name + " " + verb + "s " + discarded.aname + " token")
      return discarded
    else:
      if token in self.pool:
        self.pool.remove(token)
        if self.game.reporting:
          self.game.report(self.name + " " + verb + "s " + token.aname + " token")
        return token
      else:
        return False
  ...
  def recover_tokens(self, n):
    ...
    if self.game.reporting and gain:
      self.game.report(
        "%s gains %d %s token%s"
        % (self.name, gain, self.tokens[0].name, ("s" if gain > 1 else ""))
      )

  def spend_counters(self, effect_string, max_spend=None, set_amount=False):
    """Creates fork for spending, reports spending, and returns
        amount spent."""
    if self.counters == 0 or self.opponent.blocks_tokens():
      return 0
    ...
    if spend:
      ...
      if self.game.reporting:
        self.game.report(
          "%s spends %d %s Counter%s (now has %d)"
          % (
            self.name,
            spend,
            self.counter_name,
  ...
  def discard_counters(self, n):
    ...
    if loss and self.game.reporting:
      self.game.report(
        "%s discards %d %s Counter%s (now has %d)"
        % (self.name, loss, self.counter_name, "" if loss == 1 else "s", self.counters)
      )
    return loss
  ...
  def recover_counters(self, n):
    ...
    if gain and self.game.reporting:
      self.game.report(
        "%s recovers %d %s Counter%s (now has %d)"
        % (self.name, gain, self.counter_name, "" if gain == 1 else "s", self.counters)
      )
    return gain
  ...
  def remove_card_from_game(self, card):
    ...
    if self.game.reporting:
      self.game.report(
        "%s's %s %s is removed from the game"
  ...
  def move(self, moves, specific_movement_reaction=None):
    ...
    self.execute_move(
      self, moves, specific_movement_reaction=specific_movement_reaction
    )

  def move_opponent(self, moves, specific_movement_reaction=None):
    moves = [-m for m in moves if m != 0] + list(moves)
    self.execute_move(
      self.opponent, moves, specific_movement_reaction=specific_movement_reaction
    )

  def move_directly(self, dests, specific_movement_reaction=None):
    self.execute_move(
      self, dests, direct=True, specific_movement_reaction=specific_movement_reaction
    )
  ...
  def execute_move(
    self, mover, moves, direct=False, max_move=False, specific_movement_reaction=None
  ):
  ...
  def inner_execute_move(self, mover, moves, direct, max_move):
    ...
    mover.opponent_block = mover.opponent.blocks_opponent_movement(self, direct)
    ...
    if not blocked:
      ...
      unobstructed = self.get_unobstructed_positions(mover_pos, blocked, direct)
      possible = list(dests & unobstructed)
    if max_move and possible:
      direction = sum(moves) * (mover.opponent.position - mover.position)
      if direction > 0:
  ...
  def expected_soak(self):
    ...
    return sum(s.soak for s in styles) / len(styles) + sum(b.soak for b in bases) / len(
      bases
    )
  ...
  def expected_stunguard(self):
    ...
    styles = set(self.styles) - unavailable_cards
    bases = set(self.bases) - unavailable_cards
    return sum(s.stunguard for s in styles) / len(styles) + sum(
      b.stunguard for b in bases
    ) / len(bases)
  ...
  def set_preferred_range(self):
    ...
    styles = set(self.styles) - unavailable_cards
    bases = set(self.bases) - unavailable_cards
    self.preferred_range = sum(s.get_preferred_range() for s in styles) / float(
      len(styles)
    ) + sum(b.get_preferred_range() for b in bases) / float(len(bases))

  def evaluate_range(self):
    return -self.game.range_weight * (self.preferred_range - self.game.distance()) ** 2
  ...
  def evaluate(self):
    ...
    card_bonus = (
      0 if self.game.first_beats else sum(card.evaluation_bonus() for card in hand)
    )
    discard_penalty = (
      0
      if self.game.first_beats
      else -sum(card.get_discard_penalty() for card in self.discard[1])
      - sum(card.get_discard_penalty() for card in self.discard[2]) / 2.0
    )
    if self.special_action_available:
      special_action_bonus = self.special_action.value + (
        max([finisher.evaluate_setup() for finisher in self.finishers])
        if (self.life <= 7)
        else 0
      )
    else:
    ...
    status_effect_bonus = sum([status.value for status in self.pending_status_effects])
  ...
  def choose_finisher_base_retroactively(self):
    options = list(
      (self.finisher_generators & set(self.bases)) - (self.discard[1] | self.discard[2])
    )
    assert options
    if len(options) == 1:
      return options[0]
...
class Card(object):
  ...
  def __init__(self, the_game, my_player):
    ...
    self.name = self.name_override or re.sub(
      r"([a-z](?=[A-Z0-9])|[A-Z0-9](?=[A-Z0-9][a-z]))", r"\1 ", self.__class__.__name__
    )
...
class PowerBonusStatusEffect(StatusEffect):
  ...
  def situation_report_line(self):
    return "Power bonus: %d" % self.me.active_power_bonus

  @property
  def activation_line(self):
    return "%s will have %d power next beat" % (
      self.me.name,
      self.me.pending_power_bonus,
    )
...
class PriorityBonusStatusEffect(StatusEffect):
  ...
  @property
  def situation_report_line(self):
    return "Priority bonus: %d" % self.me.active_priority_bonus

  @property
  def activation_line(self):
    return "%s will have %d priority next beat" % (
      self.me.name,
      self.me.pending_priority_bonus,
    )
...
class PowerPenaltyStatusEffect(StatusEffect):
  ...
  @property
  def situation_report_line(self):
    return "Opponent has power penalty: %d" % self.me.active_power_penalty

  @property
  def activation_line(self):
    return "%s will have %d priority next beat" % (
      self.opponent.name,
      self.me.pending_power_penalty,
    )
...
class PriorityPenaltyStatusEffect(StatusEffect):
  ...
  @property
  def situation_report_line(self):
    return "Opponent has priority penalty: %d" % self.me.active_priority_penalty

  @property
  def activation_line(self):
    return "%s will have %d priority next beat" % (
      self.opponent.name,
      self.me.pending_priority_penalty,
    )
...
class RefinedStatusEffect(StatusEffect):
  ...
  activation_line = (
    "Jager will have +1 power, +1 priority if using a signature move next beat"
  )
...
class Abarene(Character):
  def __init__(self, the_game, n, base_set="alpha", is_user=False):
    ...
    self.styles = [
      Lethal(the_game, self),
      Intoxicating(the_game, self),
      Barbed(the_game, self),
      Crippling(the_game, self),
      Pestilent(the_game, self),
    ]
    self.finishers = [Flytrap(the_game, self), HallicrisSnare(the_game, self)]
    self.tokens = [
      Dizziness(the_game, self),
  ...
  def choose_initial_discards(self):
    return (self.lethal, self.burst, self.barbed, self.grasp)
  ...
  def recover_tokens(self, choosing_player, from_ante_only=False):
    ...
    if recoverable:
      ...
      choice = self.game.make_fork(len(recoverable), choosing_player, prompt, options)
  ...
  def evaluate(self):
    return Character.evaluate(self) + sum([token.value for token in self.pool])
...
class Adjenna(Character):
  ...
  def choose_initial_discards(self):
    return (self.beckoning, self.burst, self.alluring, self.grasp)

  def situation_report(self):
    report = Character.situation_report(self)
    report.append(
      "%d Petrification markers on %s" % (self.petrification, self.opponent.name)
    )
    return report
  ...
  def petrify(self):
    ...
    if self.game.reporting:
      self.game.report(self.opponent.name + " receives a Petrification Marker")
    if self.petrification > 5:
      if self.game.reporting:
        self.game.report(self.opponent.name + " has 6 Petrification Markers")
  ...
  def evaluate(self):
    ...
    return (
      Character.evaluate(self)
      + self.opponent.effective_life()
      - effective_life
      + 2 * self.petrification
    )
...
class Alexian(Character):
  def __init__(self, the_game, n, base_set="alpha", is_user=False):
    ...
    self.styles = [
      Gestalt(the_game, self),
      Regal(the_game, self),
      Stalwart(the_game, self),
      Mighty(the_game, self),
      Steeled(the_game, self),
    ]
    self.finishers = [EmpireDivider(the_game, self), HailTheKing(the_game, self)]
  ...
  def choose_initial_discards(self):
    return (self.mighty, self.strike, self.regal, self.grasp)
  ...
  def ante_trigger(self):
    ...
    for unused in range(self.opponent.strat[2][1]):
      ...
      if self.game.reporting:
        self.game.report("%s antes a Chivalry token" % self.opponent.name)
  ...
  def ignore_style_range(self, player):
    return player is self.opponent and self.mighty in self.active_cards
...
class Alumis(Character):
  def __init__(self, the_game, n, base_set="alpha", is_user=False):
    ...
    self.styles = [
      Tenebrous(the_game, self),
      Arcane(the_game, self),
      Horrific(the_game, self),
      Ghastly(the_game, self),
      Sinister(the_game, self),
    ]
    self.finishers = [AphoticTerror(the_game, self), UmbralVice(the_game, self)]
    Character.__init__(self, the_game, n, base_set, is_user)
  ...
  def choose_initial_discards(self):
    return (self.sinister, self.grasp, self.tenebrous, self.strike)
...
class Arec(Character):
  ...
  # TODO: choose
  def choose_initial_discards(self):
    return (self.phantom, self.strike, self.manipulative, self.grasp)
  ...
  def situation_report(self):
    ...
    if self.manipulative_base:
      report.append("Manipulative forced base: %s" % self.manipulative_base)
    return report
  ...
  def get_antes(self):
    clone_antes = [False] if self.clone_position is None else [False, True]
  ...
  def recover_tokens(self):
    ...
    if len(recoverable) > 0:
      ...
      choice = self.game.make_fork(len(recoverable), self, prompt, options, choice)
      recovered = recoverable[choice]
      self.pool += [recovered]
      if self.game.reporting:
  ...
  def evaluate_superposition(self):
    if self.clone_position is None or self.clone_position == self.opponent.position:
      return 0
...
class Aria(Character):
  def __init__(self, the_game, n, base_set="alpha", is_user=False):
    ...
    self.turret = Turret()
    self.droids = [self.dampening, self.magnetron, self.turret]

  mean_priority_bonus = 0.4  # Magnetron

  def choose_initial_discards(self):
    return (self.laser, self.grasp, self.ionic, self.dash)
  ...
  def full_save(self):
    ...
    state.droids_have_attacked = [droid.has_attacked for droid in self.droids]
  ...
  def give_power_penalty(self):
    ...
    return (
      -1
      if dampening is not None and abs(self.opponent.position - dampening) <= 1
      else 0
    )
  ...
  def give_priority_penalty(self):
    ...
    if magnetron is not None and abs(self.opponent.position - magnetron) <= 1:
      penalty -= 1
    return penalty
  ...
  def before_trigger(self):
    ...
    if self.laser in self.active_cards:
      sources = [self] + [d for d in self.droids if d.position is not None]
      ans = self.game.make_fork(len(sources), self, "Choose source of attack:", sources)
  ...
  def evaluate(self):
    ...
    magnetron = self.magnetron.position
    turret = self.turret.position
    positions = [droid.position for droid in self.droids if droid.position is not None]
...
class Baenvier(Character):
  ...
  def choose_initial_discards(self):
    return (self.destruction, self.strike, self.curse, self.grasp)
  ...
  def situation_report(self):
    report = Character.situation_report(self)
    report.append(
      "%d Spell Eater token%s" % (len(self.pool), "" if len(self.pool) == 1 else "s")
    )
    return report
  ...
  def ante_trigger(self):
    if self.strat[2][0]:
      ...
      if any(player.style is player.special_action for player in self.game.player):
        return
      ...
      for player in (self, self.opponent):
        if player.style is player.special_action:
          ...
          if player.base is player.cancel:
            ...
            if any(f.devolves_into_cancel() for f in player.finishers):
        ...
        if self.game.reporting:
          self.game.report("%s is returned to %s's hand" % (player.style, player))
  ...
  def take_a_hit_trigger(self):
    if self.runic in self.active_cards:
      self.add_triggered_power_bonus(max(0, self.opponent.get_power_bonuses()))

  def get_ante_name(self, a):
    return "Spell Eater" if a == 1 else ""

  def ignore_style_range(self, player):
    return player is self and self.spire in self.active_cards
...
class Borneo(Character):
  def __init__(self, the_game, n, base_set="alpha", is_user=False):
    ...
    self.status_effects = [OpponentPowerPriorityStatusEffect(the_game, self)]
    Character.__init__(self, the_game, n, base_set, is_user)
    self.unstunners = set([self.hex, self.clumsy])

  def choose_initial_discards(self):
    return (self.slippery, self.smoke, self.petulant, self.knuckle)
  ...
  def get_maxrange(self):
    if self.base.name == "Smoke":
      maxrange = self.base.maxrange + self.opponent.give_maxrange_penalty()
      if self.opponent.blocks_maxrange_bonuses():
  ...
  def get_minrange(self):
    if self.base.name == "Smoke":
      minrange = self.base.minrange + self.opponent.give_minrange_penalty()
      if self.opponent.blocks_minrange_bonuses():
...
class Byron(Character):
  ...
  def choose_initial_discards(self):
    return (self.soulless, self.strike, self.heartless, self.shot)
  ...
  def ignore_style_range(self, player):
    return player is self and self.smoke in self.active_cards
...
class Cadenza(Character):
  ...
  def choose_initial_discards(self):
    return (self.mechanical, self.strike, self.grapnel, self.grasp)

  def situation_report(self):
    report = Character.situation_report(self)
    report.append(
      "%d Iron Body token%s" % (len(self.pool), "" if len(self.pool) == 1 else "s")
    )
    return report
  ...
  def stun(self, damage=None):
    if (
      damage is not None
      and self.can_spend(1)
      and self.iron_body not in self.ante
      and damage > self.get_stunguard()
    ):
      if self.game.make_fork(2, self, "Spend an Iron Body token?", ["No", "Yes"]):
        self.token_spent = 1
        self.spend_token()
        if self.game.reporting:
...
class Cesar(Character):
  ...
  def choose_initial_discards(self):
    return (self.fueled, self.strike, self.unstoppable, self.grasp)
  ...
  def gain_threat_level(self):
    ...
    if self.game.reporting:
      self.game.report(
        "Cesar gains a Threat Level (current level: %d)" % self.threat_level
      )
    if self.threat_level == 4:
      self.stun()

  def get_power_bonus(self):
    return self.threat_level_at_ante if self.threat_level_at_ante < 4 else 0
  ...
  def has_stun_immunity(self):
    ...
    # play Fueled.
    return (
      self.threat_level_at_ante == 3 and self.threat_level == 3
...
class Claus(Character):
  ...
  def choose_initial_discards(self):
    return (self.hurricane, self.grasp, self.cyclone, self.shot)
  ...
  def inner_execute_move(self, mover, moves, direct, max_move):
    ...
    if not blocked:
      ...
      unobstructed = self.get_unobstructed_positions(initial_pos, blocked, direct)
      possible = list(dests & unobstructed)
    if possible:
      ...
      self.push(
        [push_distance], specific_movement_reaction=self.specific_movement_reaction
      )

  def specific_movement_reaction(self, initiator, mover, old_position, direct):
    self.deal_damage(abs(self.opponent.position - old_position))
...
class Clinhyde(Character):
  def __init__(self, the_game, n, base_set="alpha", is_user=False):
    ...
    self.finishers = [
      VitalSilverInfusion(the_game, self),
      RitherwhyteInfusionClinhyde(the_game, self),
    ]
    self.packs = [
      Crizma(the_game, self),
  ...
  def choose_initial_discards(self):
    return (self.diffusion, self.unique_base, self.phase, self.grasp)
  ...
  def situation_report(self):
    report = Character.situation_report(self)
    report.append(
      "Active Stim Packs: " + ", ".join([p.name for p in self.active_packs])
    )
...
class Clive(Character):
  ...
  def choose_initial_discards(self):
    # Not really checked
    return (self.upgradeable, self.grasp, self.rocket, self.strike)
  ...
  def read_my_state(self, lines, board, addendum):
    ...
    if lines[0] == "No modules in stack\n":
      ...
      while lines[i].startswith("  "):
        ...
        i += 1
      self.module_stack = [
        module for module in self.modules if module.name in module_names
      ]
  ...
  def return_module(self, module):
    ...
    if module in self.active_modules:
      ...
      self.set_active_cards()
    if self.game.reporting:
      self.game.report("Clive returns %s to his module stack" % module.name)
...
class Danny(Character):
  ...
  def choose_initial_discards(self):
    return (self.fallen, self.burst, self.vicious, self.shot)
  ...
  def add_monsters(self, n_monsters, positions):
    ...
    combo = combos[self.game.make_fork(len(combos), self, prompt, options)]
  ...
  def remove_monsters(self, n_monsters, positions):
    ...
    if not n_monsters:
      return
    combos = list(itertools.combinations(positions, n_monsters))
    prompt = "Remove monster:" if n_monsters == 1 else "Remove monsters:"
    options = []
    if self.is_user and self.game.interactive_mode:
      for combo in combos:
        tmp_list = ["."] * 7
  ...
  def reduce_stunguard(self, stunguard):
    return (
      0 if self.fallen_ignore_stunguard else Character.reduce_stunguard(self, stunguard)
    )

  def special_range_hit(self):
    return self.opponent.position in self.monsters or Character.special_range_hit(self)
  ...
  def evaluate_range(self):
    ...
    penalty = -self.game.range_weight * (self.preferred_range - distance) ** 2
  ...
  def evaluate(self):
    ...
    return Character.evaluate(self) + 0.1 * self.game.expected_beats() * len(
      self.monsters
    )
...
class Demitras(Character):
  def __init__(self, the_game, n, base_set="alpha", is_user=False):
    ...
    self.styles = [
      Darkside(the_game, self),
      Bloodletting(the_game, self),
      Vapid(the_game, self),
      Illusory(the_game, self),
      Jousting(the_game, self),
    ]
    self.finishers = [SymphonyOfDemise(the_game, self), Accelerando(the_game, self)]
  ...
  def choose_initial_discards(self):
    return (self.bloodletting, self.grasp, self.illusory, self.strike)
  ...
  def situation_report(self):
    report = Character.situation_report(self)
    report.append(
      "%d Crescendo token%s" % (len(self.pool), "" if len(self.pool) == 1 else "s")
    )
    return report
...
class Eligor(Character):
  ...
  def choose_initial_discards(self):
    return (self.martial, self.grasp, self.chained, self.strike)

  def situation_report(self):
    report = Character.situation_report(self)
    report.append(
      "%d Vengeance token%s" % (len(self.pool), "" if len(self.pool) == 1 else "s")
    )
    return report
  ...
  def get_minimum_life(self):
    return (
      1 if self.sweet_revenge in self.active_cards else Character.get_minimum_life(self)
    )
...
class Eustace(Character):
  def __init__(self, the_game, n, base_set="alpha", is_user=False):
    ...
    self.styles = [
      Elementary(the_game, self),
      Ivory(the_game, self),
      Frigid(the_game, self),
      Frostbite(the_game, self),
      Barons(the_game, self),
    ]
    self.finishers = [BoneBreaker(the_game, self), RimeBlitzer(the_game, self)]
  ...
  def choose_initial_discards(self):
    return (self.barons, self.grasp, self.elementary, self.drive)
  ...
  def situation_report(self):
    report = Character.situation_report(self)
    report.append(
      "%d Frostflow token%s" % (len(self.pool), "" if len(self.pool) == 1 else "s")
    )
    return report
  ...
  def set_preferred_range(self):
    ...
    styles = set(self.styles) - unavailable_cards
    bases = set(self.bases) - unavailable_cards
    preferred_range = sum(s.get_preferred_range() for s in styles) / float(
      len(styles)
    ) + sum(b.get_preferred_range() for b in bases) / float(len(bases))
    preferred_range_when_anteing = sum(
      s.get_preferred_range() - (s.maxrange - s.minrange) / 2 for s in styles
    ) / float(len(styles)) + sum(
      b.get_preferred_range() - (b.maxrange - b.minrange) / 2 for b in bases
    ) / float(
      len(bases)
    )
    ante_prob = len(self.pool) * 0.1
    self.preferred_range = (
      preferred_range * (1 - ante_prob) + preferred_range_when_anteing * ante_prob
    )
  ...
  def evaluate(self):
    return (
      Character.evaluate(self)
      + self.frost_value * self.counters
...
class Gerard(Character):
  ...
  def choose_initial_discards(self):
    return (self.villainous, self.unique_base, self.hooked, self.strike)
  ...
  def read_my_state(self, lines, board, addendum):
    ...
    if lines[1] == "No mercenaries in play\n":
      ...
      while lines[i].startswith("  "):
        ...
        i += 1
      self.mercs_in_play = [
        merc for merc in self.mercenaries if merc.name in merc_names
      ]
  ...
  def get_antes(self):
    ...
    for hiring_combo in hiring_combos:
      ...
      if hiring_combo:
        remaining_gold = hiring_gold - sum([h.hiring_cost for h in hiring_combo])
      else:
      ...
      for n_activations in range(len(possible_activations) + 1):
        ...
        playable = [
          list(c)
          for c in combos_n
          if sum([a.activation_cost for a in c]) <= remaining_gold
        ]
        antes += [(hiring_combo, activation_combo) for activation_combo in playable]
        if not playable:
          break
    return antes
  ...
  def input_ante(self, pair):
    ...
    while True:
      ...
      possible_hires = [
        ...
        if merc not in self.mercs_in_play + self.removed_mercs + ante[0]
        and merc.hiring_cost - discount <= remaining_gold
      ]
    ...
    while True:
      possible_activations = [
        merc
        for merc in self.mercs_in_play + ante[0]
        if merc not in ante[1]
        and merc.activation_cost
        and merc.activation_cost <= remaining_gold
      ]
  ...
  def ante_trigger(self):
    ...
    discount = 2 if (self.initiation_status_effect in self.active_status_effects) else 0
    for h in hire:
      self.mercs_in_play.append(h)
      self.gold -= h.hiring_cost - discount
      if self.game.reporting:
        self.game.report(
          "Gerard hires a %s for %d gold (%d remaining)"
    ...
    for a in activate:
      ...
      if self.game.reporting:
        self.game.report(
          "Gerard activates his %s for %d gold (%d remaining)"
  ...
  def set_active_cards(self):
    Character.set_active_cards(self)
    self.active_cards += [
      merc for merc in self.mercs_in_play if not merc.activation_cost
    ]
  ...
  def gain_gold(self, gold):
    ...
    if gain and self.game.reporting:
      self.game.report(
        "Gerard gains %d Gold Token%s (has %d)"
...
class Heketch(Character):
  def __init__(self, the_game, n, base_set="alpha", is_user=False):
    ...
    self.styles = [
      Merciless(the_game, self),
      Rasping(the_game, self),
      Critical(the_game, self),
      Assassin(the_game, self),
      Psycho(the_game, self),
    ]
    self.finishers = [MillionKnives(the_game, self), LivingNightmare(the_game, self)]
    self.status_effects = [OpponentImmobilizedStatusEffect(the_game, self)]
  ...
  def choose_initial_discards(self):
    return (self.rasping, self.burst, self.critical, self.strike)
  ...
  def read_my_state(self, lines, board, addendum):
    lines = Character.read_my_state(self, lines, board, addendum)
    self.pool = [self.dark_force] if find_start(lines, "Dark Force") else []
...
class Hepzibah(Character):
  ...
  # TODO: choose
  def choose_initial_discards(self):
    return (self.pactbond, self.unique_base, self.anathema, self.strike)
  ...
  def get_antes(self):
    ...
    if self.active_pactbond:
      combos = [c for c in combos if self.active_pactbond in c]
    return combos
...
class Hikaru(Character):
  ...
  def choose_initial_discards(self):
    return (self.sweeping, self.grasp, self.trance, self.strike)
...
class Iri(Character):
  def __init__(self, the_game, n, base_set="alpha", is_user=False):
    ...
    self.finishers = [
      LimitExceed(the_game, self),
      FinalGalaxySpearAtamos(the_game, self),
    ]
    self.forms = [
      MagicalForm(the_game, self),
  ...
  def choose_initial_discards(self):
    return (self.galactic, self.burst, self.aurora, self.grasp)
...
class Jager(Character):
  ...
  def set_active_cards(self):
    if self.strat[2][0] and not self.opponent.blocks_tokens():
      ...
      self.active_cards = (
        [self.base] + self.active_status_effects + self.get_active_tokens()
      )
    else:
      Character.set_active_cards(self)
  ...
  def choose_initial_discards(self):
    return (self.refined, self.grasp, self.menacing, self.burst)
  ...
  def get_strategies(self):
    ...
    return [s for s in strats if s[0] is not self.special_action or s[2][0] is None]
  ...
  def evaluate(self):
    return Character.evaluate(self) + sum(s.get_value() for s in self.signature_pool)
...
class Juto(Character):
  ...
  def choose_initial_discards(self):
    return (self.snuggly, self.flash, self.cuddly, self.whirlpool)
...
class Kajia(Character):
  def __init__(self, the_game, n, base_set="alpha", is_user=False):
    ...
    self.finishers = [Wormwood(the_game, self), ImagoEmergence(the_game, self)]
    Character.__init__(self, the_game, n, base_set, is_user)

  def choose_initial_discards(self):
    return (self.stinging, self.strike, self.swarming, self.burst)
  ...
  def situation_report(self):
    report = Character.situation_report(self)
    report.append("%d insects on %s's discard 1" % (self.insects[1], self.opponent))
    report.append("%d insects on %s's discard 2" % (self.insects[2], self.opponent))
  ...
  def read_my_state(self, lines, board, addendum):
    ...
    self.imago_emergence_active = find_start(lines, "Imago Emergence is active")
  ...
  def give_insects(self, n, pile=0):
    if n > 0:
      ...
      if self.game.reporting:
        ...
        self.game.report("(Current insects: %d / %d / %d)" % tuple(self.insects))
  ...
  def reduce_stunguard(self, stunguard):
    return (
      0 if self.stinging_ignores > 1 else Character.reduce_stunguard(self, stunguard)
    )
...
class Kallistar(Character):
  def __init__(self, the_game, n, base_set="alpha", is_user=False):
    ...
    self.finishers = [Supernova(the_game, self), ChainOfDestruction(the_game, self)]
    self.status_effects = [PriorityPenaltyStatusEffect(the_game, self)]
    self.forms = [HumanForm(the_game, self), ElementalForm(the_game, self)]
    Character.__init__(self, the_game, n, base_set, is_user)
  ...
  def choose_initial_discards(self):
    return (self.flare, self.strike, self.caustic, self.drive)
  ...
  def read_my_state(self, lines, board, addendum):
    lines = Character.read_my_state(self, lines, board, addendum)
    self.form = (
      self.elemental_form if find_start(lines, "Elemental") else self.human_form
    )
...
class Karin(Character):
  def __init__(self, the_game, n, base_set="alpha", is_user=False):
    ...
    self.styles = [
      Howling(the_game, self),
      Coordinated(the_game, self),
      FullMoon(the_game, self),
      Feral(the_game, self),
      Dual(the_game, self),
    ]
    self.finishers = [RedMoonRage(the_game, self), LunarCross(the_game, self)]
    Character.__init__(self, the_game, n, base_set, is_user)
  ...
  def choose_initial_discards(self):
    return (self.dual, self.grasp, self.howling, self.strike)
  ...
  def move_jager(self, moves):
    ...
    self.jager_position = positions[
      self.game.make_fork(len(positions), self, prompt, options)
    ]
  ...
  def attack_range(self):
    attack_source = (
      self.jager_position if self.howling in self.active_cards else self.position
    )
  ...
  def set_preferred_range(self):
    ...
    self.preferred_range = (
      sum(s.get_preferred_range() for s in karin_styles) / self.karin_weight
    ) + base_range
    self.jager_range = (
      (sum(s.get_preferred_range() for s in jager_styles) / self.jager_weight)
      + base_range_jager
      if self.jager_weight > 0
      else 0
    )
  ...
  def evaluate_range(self):
    karin_penalty = (
      self.karin_weight * (self.preferred_range - self.game.distance()) ** 2
    )
    jager_penalty = (
      self.jager_weight
      * (self.jager_range - (abs(self.jager_position - self.opponent.position))) ** 2
    )
    return (
      -self.game.range_weight
      * (karin_penalty + jager_penalty)
...
class Kehrolyn(Character):
  ...
  def choose_initial_discards(self):
    return (self.mutating, self.grasp, self.bladed, self.strike)
  ...
  def set_preferred_range(self):
    Character.set_preferred_range(self)
    self.preferred_range += (
      self.whip.preferred_range if self.whip in self.discard[1] else 0
    )
...
class Khadath(Character):
  ...
  def choose_initial_discards(self):
    ...
    return (self.evacuation, self.grasp, self.hunters, self.unique_base)
  ...
  def blocks_opponent_movement(self, initiator, direct):
    if (
      direct
      or initiator is self
      or self.trap_position == self.position
      or self.trap_position is None
    ):
      return set()
    return set(
      [
...
class Larimore(Character):
  def __init__(self, the_game, n, base_set="alpha", is_user=False):
    ...
    self.styles = [
      Dragonbreath(the_game, self),
      Blistering(the_game, self),
      Warmages(the_game, self),
      Dire(the_game, self),
      Incinerating(the_game, self),
    ]
    self.finishers = [Dragonflare(the_game, self), TheJump(the_game, self)]
    Character.__init__(self, the_game, n, base_set, is_user)
  ...
  def choose_initial_discards(self):
    return (self.incinerating, self.might, self.blistering, self.burst)
  ...
  def execute_move(
    ...
    if self.strat[2][0] == 3:
      return
    Character.execute_move(
      self, mover, moves, direct, max_move, specific_movement_reaction
    )
...
class Lesandra(Character):
  ...
  def choose_initial_discards(self):
    return (self.guardian, self.unique_base, self.pactbreaker, self.grasp)
  ...
  def end_trigger(self):
    ...
    available_familiars = [f for f in self.familiars if f is not unavailable]
    prompt = "Choose familiar to summon:"
    damage = self.opponent.damage_taken
    if self.game.interactive_mode and self.is_user:
      options = [
        "%s (cost: %d life)" % (f.name, f.get_cost() if f.get_cost() > damage else 0)
        for f in available_familiars
      ]
      options.append("None")
    else:
      options = []
    ans = self.game.make_fork(len(available_familiars) + 1, self, prompt, options)
...
class Lixis(Character):
  ...
  def choose_initial_discards(self):
    return (self.pruning, self.grasp, self.rooted, self.strike)
  ...
  def discard_opponent_base(self):
    ...
    discard = bases[self.game.make_fork(len(bases), self.opponent, prompt, options)]
  ...
  def give_priority_penalty(self):
    ...
    if (
      self.virulent_miasma_active
      and self.game.initial_state.player_states[1 - self.my_number].life == 1
    ):
      penalty -= 3
    return penalty
...
class Luc(Character):
  ...
  def choose_initial_discards(self):
    return (self.eternal, self.flash, self.fusion, self.strike)

  def situation_report(self):
    report = Character.situation_report(self)
    report.append(
      "%d Time token%s" % (len(self.pool), "" if len(self.pool) == 1 else "s")
    )
    return report
  ...
  def set_active_cards(self):
    ...
    self.active_cards = [card for card in self.active_cards if card is not self.time]
    if n_tokens == 1:
...
class Lymn(Character):
  def __init__(self, the_game, n, base_set="alpha", is_user=False):
    ...
    self.styles = [
      Maddening(the_game, self),
      Chimeric(the_game, self),
      Surreal(the_game, self),
      Reverie(the_game, self),
      Fathomless(the_game, self),
    ]
    self.finishers = [Megrim(the_game, self), Conceit(the_game, self)]
    Character.__init__(self, the_game, n, base_set, is_user)

  def choose_initial_discards(self):
    return (self.maddening, self.burst, self.chimeric, self.dash)
...
class Magdelina(Character):
  def __init__(self, the_game, n, base_set="alpha", is_user=False):
    ...
    self.finishers = [SolarSoul(the_game, self), Apotheosis(the_game, self)]
    Character.__init__(self, the_game, n, base_set, is_user)
  ...
  def choose_initial_discards(self):
    return (self.spiritual, self.shot, self.sanctimonious, self.blessing)
  ...
  def ante_trigger(self):
    ...
    if self.level != 0 and self.game.reporting:
      self.game.report(
        "Magdelina gets a +%d bonus to power, priority and stunguard" % self.level
      )
  ...
  def discard_trance(self, n):
    ...
    if self.game.reporting:
      self.game.report(
        "%s discards %d Trance Counter%s (now has %d)"
...
class Marmelee(Character):
  def __init__(self, the_game, n, base_set="alpha", is_user=False):
    ...
    self.styles = [
      Petrifying(the_game, self),
      Magnificent(the_game, self),
      Sorceress(the_game, self),
      Barrier(the_game, self),
      Nullifying(the_game, self),
    ]
    self.finishers = [AstralCannon(the_game, self), AstralTrance(the_game, self)]
    Character.__init__(self, the_game, n, base_set, is_user)
    self.counter_name = "Concentration"
    self.max_counters = 5

  def choose_initial_discards(self):
    return (self.petrifying, self.burst, self.barrier, self.grasp)
...
class Mikhail(Character):
  def __init__(self, the_game, n, base_set="alpha", is_user=False):
    ...
    self.styles = [
      Immutable(the_game, self),
      Transcendent(the_game, self),
      Hallowed(the_game, self),
      Apocalyptic(the_game, self),
      Sacred(the_game, self),
    ]
    self.finishers = [MagnusMalleus(the_game, self), TheFourthSeal(the_game, self)]
  ...
  def choose_initial_discards(self):
    return (self.hallowed, self.grasp, self.apocalyptic, self.strike)
  ...
  def situation_report(self):
    report = Character.situation_report(self)
    report.append(
      "%d Seal token%s" % (len(self.pool), "" if len(self.pool) == 1 else "s")
    )
    return report
  ...
  def set_active_cards(self):
    Character.set_active_cards(self)
    if not (self.seal in self.ante or isinstance(self.style, SpecialAction)):
...
class Oriana(Character):
  def __init__(self, the_game, n, base_set="alpha", is_user=False):
    ...
    self.styles = [
      Celestial(the_game, self),
      Stellar(the_game, self),
      Unstable(the_game, self),
      Metamagical(the_game, self),
      Calamity(the_game, self),
    ]
    self.finishers = [NihilEraser(the_game, self), GalaxyConduit(the_game, self)]
  ...
  def choose_initial_discards(self):
    return (self.stellar, self.grasp, self.calamity, self.strike)

  def situation_report(self):
    report = Character.situation_report(self)
    report.append(
      "%d Magic Point token%s" % (len(self.pool), "" if len(self.pool) == 1 else "s")
    )
    return report
...
class Ottavia(Character):
  ...
  def choose_initial_discards(self):
    return (self.cybernetic, self.strike, self.demolition, self.shot)
  ...
  def give_priority_penalty(self):
    ...
    if self.target_lock is None:
      ...
      self.target_lock = guess is True or guess is self.opponent_priority_pre_penalty
  ...
  def post_simulation_processing(self):
    ...
    strats_without_guesses = [
      (s, i) for i, s in enumerate(fake_strats) if s[2][0] is False
    ]
    # Then, make a new strategy list, with one guess for each
    # possible opponent priority (keeping the index of the original
    # fake strat).
    full_strats = [
      ((s[0][0], s[0][1], (p, s[0][2][1], None)), s[1])
      for s in strats_without_guesses
      for p in priorities
    ]
    ...
    if self.my_number == 0:
      ...
      for i in range(n):
        for j in range(m):
          ...
          is_correct = (
            my_strat[2][0]
            == fake_results[my_fake_index][j][1]
            .player_states[0]
            .opponent_priority_pre_penalty
          )
          fake_strat = (my_strat[0], my_strat[1], (is_correct, my_strat[2][1], None))
      ...
      for i in range(m):
        for j in range(n):
          ...
          is_correct = (
            my_strat[2][0]
            == fake_results[i][my_fake_index][1]
            .player_states[1]
            .opponent_priority_pre_penalty
          )
          fake_strat = (my_strat[0], my_strat[1], (is_correct, my_strat[2][1], None))
...
class Rexan(Character):
  ...
  def choose_initial_discards(self):
    return (self.unyielding, self.drive, self.enervating, self.strike)
  ...
  def ante_trigger(self):
    ...
    for unused in range(self.opponent.strat[2][1]):
      ...
      if self.game.reporting:
        self.game.report("%s antes a Curse token" % self.opponent.name)
  ...
  def take_a_hit_trigger(self):
    if not self.opponent.hits_scored and self.curse not in self.opponent.ante:
      self.give_induced_tokens(1)
...
class Rukyuk(Character):
  def __init__(self, the_game, n, base_set="alpha", is_user=False):
    ...
    self.styles = [
      Sniper(the_game, self),
      PointBlank(the_game, self),
      Gunner(the_game, self),
      Crossfire(the_game, self),
      Trick(the_game, self),
    ]
    self.finishers = [FullyAutomatic(the_game, self), ForceGrenade(the_game, self)]
    self.tokens = [
      APShell(the_game, self),
  ...
  def choose_initial_discards(self):
    return (self.crossfire, self.shot, self.trick, self.reload)
  ...
  def can_hit(self):
    return Character.can_hit(self) and (
      len(set(self.ante) & set(self.tokens)) > 0 or self.base.name == "Force Grenade"
    )
  ...
  def evaluate(self):
    return (
      Character.evaluate(self)
      + sum(t.value for t in self.pool)
...
class Runika(Character):
  def __init__(self, the_game, n, base_set="alpha", is_user=False):
    ...
    self.styles = [
      Channeled(the_game, self),
      Maintenance(the_game, self),
      Explosive(the_game, self),
      Impact(the_game, self),
      Overcharged(the_game, self),
    ]
    self.finishers = [ArtificeAvarice(the_game, self), UdstadBeam(the_game, self)]
    self.artifacts = [
      Autodeflector(the_game, self),
  ...
  def choose_initial_discards(self):
    return (self.channeled, self.grasp, self.impact, self.strike)
  ...
  def situation_report(self):
    ...
    removed = set(self.artifacts) - self.active_artifacts - self.deactivated_artifacts
    if removed:
  ...
  def read_my_state(self, lines, board, addendum):
    ...
    if lines[0] == "No active artifacts\n":
      ...
      while i < len(lines) and lines[i].starts_with("  "):
        ...
        i += 1
      self.active_artifacts = set([a for a in self.artifacts if a.name in names])
    ...
    self.deactivated_artifacts = set(self.artifacts) - self.active_artriacts - removed
  ...
  def deactivation_fork(self, player, fake=False):
    ...
    if fake:
      ...
      choice = None
    ans = self.game.make_fork(len(targets), player, prompt, options, choice)
  ...
  def take_a_hit_trigger(self):
    if self.opponent.hits_scored or self.base.name == "Artifice Avarice":
      return
  ...
  def lose_life(self, life):
    if (
      self.autodeflector is self.overcharged_artifact
      and self.autodeflector in self.active_artifacts
    ):
      if self.game.reporting:
...
class Sarafina(Character):
  def __init__(self, the_game, n, base_set="alpha", is_user=False):
    ...
    self.finishers = [
      RitherwhyteInfusionSarafina(the_game, self),
  ...
  def choose_initial_discards(self):
    return (self.hollow, self.grasp, self.locus, self.field)
  ...
  def project(self, n):
    ...
    self.projection = positions[
      self.game.make_fork(len(positions), self, "Place projection:", options)
    ]
    if self.game.reporting:
  ...
  def evaluate_superposition(self):
    if self.opponent.position is None:
      return 0
    if self.projection is None or self.projection in (
...
class Seth(Character):
  ...
  def choose_initial_discards(self):
    return (self.fools, self.grasp, self.wyrding, self.strike)
  ...
  def start_trigger(self):
    if not self.opponent.blocks_start_triggers() and self.wyrding in self.active_cards:
      old_base = self.base
      ...
      if self.base != old_base:
        ...
        if old_base.name in ["Omen", "Burst", "Spike", "Force"]:
          prompt = "Execute %s's Start of Beat effect before switching?" % old_base.name
          options = ["No", "Yes"]
  ...
  def post_simulation_processing(self):
    bases = (
      set(self.opponent.bases) - self.opponent.discard[1] - self.opponent.discard[2]
    )
    ...
    strats_without_guesses = [s for s in fake_strats if s[2][0] is False]
    # Then, make a new strategy list, with 5 actual guess per strategy.
    full_strats = [
      (s[0], s[1], (b, s[2][1], None)) for s in strats_without_guesses for b in bases
    ]
...
class Shekhtur(Character):
  ...
  def choose_initial_discards(self):
    return (self.combination, self.grasp, self.reaver, self.shot)
  ...
  def situation_report(self):
    report = Character.situation_report(self)
    report.append(
      "%d Malice token%s" % (len(self.pool), "" if len(self.pool) == 1 else "s")
    )
  ...
  def evaluate(self):
    return Character.evaluate(self) + 0.3 * len(self.pool) + 5 * self.coffin_nails_hit
...
class Tanis(Character):
  ...
  # TODO: choose
  def choose_initial_discards(self):
    return (self.climactic, self.grasp, self.playful, self.unique_base)
  ...
  def initial_save(self):
    state = Character.initial_save(self)
    state.puppet_positions = [puppet.position for puppet in self.puppets]
  ...
  def get_pre_attack_decisions(self):
    options = [
      ...
      if p.position != self.opponent.position and p is not self.possessed_puppet
    ]
  ...
  def get_superposed_positions(self):
    return list(set(puppet.position for puppet in self.get_pre_attack_decisions()))
  ...
  def blocks_priority_bonuses(self):
    return self.playful in self.active_cards and not self.playful.suspend_blocking
...
class Tatsumi(Character):
  def __init__(self, the_game, n, base_set="alpha", is_user=False):
    ...
    self.finishers = [TsunamisCollide(the_game, self), GuardianXandjinn(the_game, self)]
    Character.__init__(self, the_game, n, base_set, is_user)
  ...
  def choose_initial_discards(self):
    return (self.fearless, self.burst, self.riptide, self.strike)
  ...
  def zone_3(self):
    ...
    return (
      ...
      * (self.position - self.opponent.position)
      <= 0
    )
  ...
  def can_be_hit(self):
    return Character.can_be_hit(self) and (
      not (self.riptide_dodge and self.opponent.attack_range() >= 3)
    )
  ...
  def set_preferred_range(self):
    ...
    self.preferred_range = (
      sum(s.get_preferred_range() for s in tatsumi_styles) / self.tatsumi_weight
    ) + base_range
    self.juto_range = (
      (sum(s.get_preferred_range() for s in juto_styles) / self.juto_weight)
      + base_range_juto
      if self.juto_weight > 0
      else 0
    )
  ...
  def evaluate_range(self):
    ...
    tatsumi_penalty = (
      self.tatsumi_weight * (self.preferred_range - self.game.distance()) ** 2
    )
    juto_penalty = (
      self.juto_weight
      * (self.juto_range - (abs(self.juto_position - self.opponent.position))) ** 2
    )
    return (
      -self.game.range_weight
      * (tatsumi_penalty + juto_penalty)
...
class Vanaah(Character):
  def __init__(self, the_game, n, base_set="alpha", is_user=False):
    ...
    self.styles = [
      Reaping(the_game, self),
      Judgment(the_game, self),
      Glorious(the_game, self),
      Paladin(the_game, self),
      Vengeance(the_game, self),
    ]
    self.finishers = [DeathWalks(the_game, self), HandOfDivinity(the_game, self)]
    self.status_effects = [PriorityPenaltyStatusEffect(the_game, self)]
  ...
  def choose_initial_discards(self):
    return (self.vengeance, self.burst, self.reaping, self.strike)
  ...
  def cycle(self):
    ...
    if self.divine_rush in self.discard[2] and not isinstance(
      self.style, SpecialAction
    ):
      self.pool = [self.divine_rush]
    # ... and moving from 0 to 1 with finisher/pulse
    # (which don't cycle).
    if self.style is self.special_action and self.divine_rush in self.discard[0]:
...
class Voco(Character):
  def __init__(self, the_game, n, base_set="alpha", is_user=False):
    ...
    self.styles = [
      Monster(the_game, self),
      Metal(the_game, self),
      Hellraising(the_game, self),
      Abyssal(the_game, self),
      Thunderous(the_game, self),
    ]
    self.finishers = [ZMosh(the_game, self), TheWave(the_game, self)]
    Character.__init__(self, the_game, n, base_set, is_user)
  ...
  def choose_initial_discards(self):
    return (self.thunderous, self.unique_base, self.monster, self.burst)
  ...
  def unique_ability_end_trigger(self):
    ...
    if opp in self.zombies:
      if self.game.make_fork(
        2, self, "Remove zombie to cause 1 life loss?", ["No", "Yes"]
      ):
        self.opponent.lose_life(1)
...
class Xenitia(Character):
  def __init__(self, the_game, n, base_set="alpha", is_user=False):
    ...
    self.styles = [
      Draining(the_game, self),
      Guilty(the_game, self),
      Razor(the_game, self),
      Rending(the_game, self),
      Purging(the_game, self),
    ]
    self.finishers = [Exsanguinate(the_game, self), Phlebotomy(the_game, self)]
    Character.__init__(self, the_game, n, base_set, is_user)
  ...
  def choose_initial_discards(self):
    return (self.rending, self.burst, self.razor, self.grasp)
  ...
  def take_damage(self, damage):
    ...
    # Xenitia's addition
    if self.guilty in self.active_cards and self.strat[2][0] == final_damage:
      final_damage = 0
  ...
  def reduce_stunguard(self, stunguard):
    return (
      0 if self.razor_ignore_stunguard else Character.reduce_stunguard(self, stunguard)
    )
...
class Zaamassal(Character):
  def __init__(self, the_game, n, base_set="alpha", is_user=False):
    ...
    self.finishers = [OpenTheGate(the_game, self), PlaneDivider(the_game, self)]
    Character.__init__(self, the_game, n, base_set, is_user)
  ...
  def choose_initial_discards(self):
    return (self.malicious, self.grasp, self.sinuous, self.burst)
  ...
  def read_my_state(self, lines, board, addendum):
    lines = Character.read_my_state(self, lines, board, addendum)
    self.active_paradigms = set([p for p in self.paradigms if p.name in lines[0]])
  ...
  def evaluate(self):
    return Character.evaluate(self) + sum(p.evaluate() for p in self.active_paradigms)
...
class Parry(Base):
  ...
  def can_be_hit(self):
    return (1 + self.me.get_priority() + self.opponent.get_priority()) % 2
...
class Catch(Base):
  ...
  def power(self):
    return (
      0
      if self.opponent.base.printed_power is None
      else self.opponent.base.printed_power
    )
...
class Barbed(Style):
  ...
  def evaluation_bonus(self):
    return 0.1 if (self.game.distance() == 2 and self.opponent.life > 3) else -0.3
...
class Nausea(Token):
  ...
  def value(self):
    ...
    return min(
      0.6,
      0.1 * self.opponent.expected_stunguard() + 0.35 * self.opponent.expected_soak(),
    )
...
class HailTheKing(Finisher):
  ...
  def before_trigger(self):
    self.me.advance(
      [0, 1, 2, 3, 4, 5], specific_movement_reaction=self.specific_movement_reaction
    )
...
class AphoticTerror(Finisher):
  ...
  def evaluate_setup(self):
    return 1 if abs(self.opponent.position - self.me.shadow_position) == 1 else 0
...
class Sinister(Style):
  ...
  def evaluation_bonus(self):
    return -0.05 if self.opponent.position == self.me.shadow_position else 0.02
...
class Tenebrous(Style):
  ...
  def evaluation_bonus(self):
    return (
      -0.1
      if self.me.shadow_position in (self.me.position, self.opponent.position)
      else 0.05
    )
...
class Phantom(Style):
  ...
  def evaluation_bonus(self):
    return (
      0.05 if len(self.me.pool) < 4 else -0.1
    ) + 0.1 * self.opponent.expected_soak()
...
class Manipulative(Style):
  ...
  def hit_trigger(self):
    ...
    if self.game.reporting:
      self.game.report(
        "Arec chooses %s's base: %s" % (self.opponent, self.me.manipulative_base)
      )
...
class LaserLattice(Finisher):
  ...
  def after_trigger(self):
    droids = [
      ...
      if droid.position is not None and not droid.has_attacked
    ]
    if not droids:
      return
    ans = self.game.make_fork(
      len(droids), self.me, "Choose droid that attacks next:", droids
    )
    droid = droids[ans]
...
class Reconfiguration(Base):
  ...
  def special_range_hit(self):
    return self.opponent.position in [droid.position for droid in self.me.droids]
  ...
  def before_trigger(self):
    ...
    if droids:
      ans = self.game.make_fork(
        len(droids), self.me, "Choose droid to be added to board:", droids
      )
      droid = droids[ans]
      ...
      ans = self.game.make_fork(
        len(positions), self.me, "Choose a space for your %s droid" % droid, options
      )
  ...
  def evaluation_bonus(self):
    return (
      0.25
      if self.opponent.position in [droid.position for droid in self.me.droids]
      else -0.25
    )
...
class Photovoltaic(Style):
  ...
  def hit_trigger(self):
    ...
    for droid in droids:
      if self.game.make_fork(
        2, self.me, "Remove %s droid from the board?" % droid, ["No", "Yes"]
      ):
        droid.position = None
        if self.game.reporting:
  ...
  def after_trigger(self):
    ...
    if droids:
      ans = self.game.make_fork(
        len(droids), self.me, "Choose droid to add to Aria's space:", droids
      )
      droid = droids[ans]
...
class Ionic(Style):
  ...
  def evaluation_bonus(self):
    return (
      0.1 if self.me.magnetron.position not in (None, self.opponent.position) else -0.1
    )
...
class Laser(Style):
  def end_trigger(self):
    if (
      self.me.turret.position is not None
      and abs(self.opponent.position - self.me.turret.position) == 3
    ):
      self.opponent.lose_life(3)
  ...
  def evaluation_bonus(self):
    ...
    value += 0.25 if turret is not None and abs(her - turret) == 3 else -0.05
...
class Spellforge(Style):
  ...
  def hit_trigger(self):
    ...
    if self.game.reporting:
      self.game.report(
        "%s is returned to %s's hand" % (self.opponent.style, self.opponent)
      )
...
class Destruction(Style):
  ...
  def evaluation_bonus(self):
    return (
      0.1 * self.opponent.expected_soak() + 0.03 * self.opponent.expected_stunguard()
    ) - 0.1
...
class Heartless(Style):
  ...
  def evaluation_bonus(self):
    return 0.2 * self.opponent.expected_soak() if self.game.distance() == 3 else 0
...
class RocketPress(Finisher):
  ...
  def evaluate_setup(self):
    return 2 if self.game.distance() > 1 or self.opponent.position in (0, 6) else 0
...
class Tempest(Base):
  ...
  def evaluation_bonus(self):
    push = min(4 - self.game.distance(), self.opponent.retreat_range())
...
class Tailwind(Style):
  ...
  def evaluation_bonus(self):
    push = min(3 - self.game.distance(), self.opponent.retreat_range())
...
class Blast(Style):
  ...
  def evaluation_bonus(self):
    push = min(2 - self.game.distance(), self.opponent.retreat_range())
...
class Frenzy(Base):
  ...
  def after_trigger(self):
    ...
    if active:
      combos = sum(
        (list(itertools.combinations(active, n)) for n in range(len(active) + 1)), []
      )
      prompt = "Choose Stim Packs to deactivate:"
      options = [", ".join([pack.name for pack in combo]) for combo in combos]
      options[0] = "None"
...
class SystemShock(Finisher):
  ...
  def devolves_into_cancel(self):
    return (
      Finisher.devolves_into_cancel(self)
      or self.me.module_stack
      or self.me.active_modules
    )
  ...
  def evaluate_setup(self):
    return (
      ...
      and not (self.me.module_stack or self.me.active_modules)
      else 0
    )
...
class Wrench(Base):
  ...
  def after_trigger(self):
    ...
    if self.game.make_fork(
      2, self.me, "Return all modules to your module stack?", ["No", "Yes"]
    ):
...
class Fallen(Style):
  ...
  def evaluation_bonus(self):
    return 0.3 + (
      0.03 * self.opponent.expected_stunguard() - 0.03
      if self.opponent.position in self.me.monsters
      else -0.33
    )
...
class Shackled(Style):
  ...
  def evaluation_bonus(self):
    ...
    return (
      0.3 * len(adjacent & self.me.monsters)
      + 0.1 * len(two_away & self.me.monsters)
      - 0.3
    )
...
class Vicious(Style):
  ...
  def get_priority_bonus(self):
    me = self.me.position
    opp = self.opponent.position
    return len([m for m in self.me.monsters if (me - opp) * (m - opp) > 0])

  def hit_trigger(self):
    me = self.me.position
    opp = self.opponent.position
    self.me.add_triggered_power_bonus(
      len([m for m in self.me.monsters if (me - opp) * (m - opp) < 0])
    )
...
class Accelerando(Finisher):
  ...
  def evaluate_setup(self):
    ...
    if (
      ...
      or self.opponent.position in [2, 3, 4]
      or (self.opponent.position == 1 and self.me.position == 0)
      or (self.opponent.position == 5 and self.me.position == 6)
    ):
...
class Illusory(Style):
  ...
  def can_be_hit(self):
    return (
      self.me.get_priority() >= self.opponent.get_priority() or len(self.me.pool) > 2
    )
...
class SweetRevenge(Finisher):
  ...
  def power(self):
    return (
      0
      if self.opponent.base.printed_power is None
      else 3 * (self.opponent.get_printed_power())
    )
...
class Aegis(Base):
  ...
  def power(self):
    return (
      0
      if self.opponent.base.printed_power is None
      else (self.opponent.get_printed_power())
    )
...
class Chained(Style):
  ...
  def before_trigger(self):
    if self.me.can_spend(1):
      ...
      pull = self.game.make_fork(
        max_pull + 1, self.me, "Choose number of Vengeance tokens to spend for pulling:"
      )
...
class Retribution(Style):
  ...
  def before_trigger(self):
    if self.opponent.hits_scored:
      self.me.move_directly(
        (self.me.position, self.opponent.position - 1, self.opponent.position + 1)
      )
...
class Villainous(Style):
  ...
  def evaluation_bonus(self):
    ...
    return (
      value * 0.25
      if self.opponent.retreat_range() <= 2 and self.me.retreat_range() >= 2
      else -value * 0.75
    )
...
class Rasping(Style):
  ...
  def hit_trigger(self):
    if self.game.distance() == 1 and self.me.can_spend(1):
      if self.game.make_fork(
        2, self.me, "Spend Dark Force Token for +3 power?", ["No", "Yes"]
      ):
        self.me.spend_token()
        self.me.add_triggered_power_bonus(3)
...
class Psycho(Style):
  ...
  def end_trigger(self):
    if self.game.distance() == 1 and self.me.can_spend(1):
      if self.game.make_fork(
        2, self.me, "Spend Dark Force Token to activate again?", ["No", "Yes"]
      ):
        self.me.spend_token()
        if self.game.reporting:
...
class Pactbond(Style):
  ...
  def end_trigger(self):
    ...
    if self.game.reporting:
      self.game.report(
        "Hepzibah Chooses %s as her free Pact" % self.me.pending_pactbond
      )
...
class FourWinds(Finisher):
  ...
  def evaluate_setup(self):
    return self.game.distance() <= 2 and max(
      0, 2 - len(self.me.pool) - len(set(self.me.ante) & set(self.me.tokens))
    )
...
class FinalGalaxySpearAtamos(Finisher):
  ...
  def can_be_hit(self):
    return (
      self.opponent.attack_range() < 4
      or self.me.current_form is not self.me.cosmic_form
    )
...
class Starlight(Base):
  ...
  def get_stunguard(self):
    return (
      2 if self.me.current_form is self.me.knight_form or self.me.limit_exceeded else 0
    )
...
class SignOfTheWolf(Finisher):
  ...
  @property
  def is_attack(self):
    return self.me.finisher_signature and self.me.finisher_signature.is_attack

  @property
  def deals_damage(self):
    return self.me.finisher_signature and self.me.finisher_signature.deals_damage

  def after_trigger(self):
    if self.me.finisher_signature:
      return
    ans = self.game.make_fork(
      1 + len(self.me.signatures),
      self.me,
...
class Menacing(Style):
  ...
  def evaluation_bonus(self):
    opp_names = [
      b.alpha_name
      for b in self.opponent.bases
      if b not in self.opponent.discard[1] | self.opponent.discard[2]
    ]
    my_names = [
      b.alpha_name
      for b in self.me.discard[1] | self.me.discard[2]
...
class Fanged(Style):
  ...
  def get_preferred_range(self):
    ...
    return 2 if (self.game.distance() == 1 and self.opponent.retreat_range() > 1) else 3
...
class SpellboltJuto(Base):
  ...
  def hit_trigger(self):
    ...
    options = ["Pull %s" % self.opponent, "Give %s -2 power" % self.opponent]
...
class Wormwood(Finisher):
  ...
  def evaluate_setup(self):
    insects = self.me.total_insects()
    return 0.25 * max(0, insects - 1) if self.game.distance() <= insects + 1 else 0
...
class Mandibles(Base):
  ...
  def evaluation_bonus(self):
    ...
    return (0.25 if stun_potential else -0.25) + insect_value
...
class Biting(Style):
  ...
  def evaluation_bonus(self):
    insect_potential = self.game.distance() <= 2 and not self.me.position in (0, 6)
    insect_value = 0.3 * (insect_potential - 0.5)
    return insect_value + 0.15 * (self.me.infested_piles() - 1)
...
class Supernova(Finisher):
  ...
  def evaluate_setup(self):
    return (
      ...
      and self.game.distance() <= 2
      and self.opponent.life <= 8
      else 0
    )
...
class ChainOfDestruction(Finisher):
  ...
  def hit_trigger(self):
    if self.me.life >= 4:
      if self.game.make_fork(
        2, self.me, "Spend 3 life to repeat this attack?", ("No", "Yes")
      ):
        self.me.lose_life(3)
        self.me.max_attacks += 1
  ...
  def evaluate_setup(self):
    if self.game.distance() >= 4 and self.me.form.is_human:
      return 2.5 if self.me.life >= 7 else (1.5 if self.me.life >= 4 else 0.5)
    else:
      return 0
...
class RedMoonRage(Finisher):
  ...
  def special_range_hit(self):
    return (
      abs(self.me.position - self.me.jager_position) == 2
      and self.me.position + self.me.jager_position == 2 * self.opponent.position
    )

  def evaluate_setup(self):
    return (
      3
      if abs(self.me.position - self.me.jager_position) == 2
      and self.me.position + self.me.jager_position == 2 * self.opponent.position
      else 0
    )
...
class LunarCross(Finisher):
  ...
  def before_trigger(self):
    if self.me.jager_position not in [self.me.position, self.opponent.position]:
      old_pos = self.me.position
...
class Claw(Base):
  ...
  def evaluation_bonus(self):
    return 0.15 if self.opponent.position == self.me.jager_position else -0.05
...
class Howling(Style):
  ...
  def evaluation_bonus(self):
    return 0.2 if self.opponent.position == self.me.jager_position else -0.1
...
class Coordinated(Style):
  ...
  def movement_reaction(self, initiator, mover, old_position, direct):
    ...
    if self.me.jager_position not in unobstructed:
      return
    if self.game.reporting:
      self.game.report("%s is blocked by Jager" % self.opponent.name)
...
class FullMoon(Style):
  ...
  def soak(self):
    return (
      2
      if (
        (self.me.position - self.me.opponent.position)
        * (self.me.jager_position - self.me.opponent.position)
        > 0
        and self.me.position != self.me.jager_position
      )
      else 0
    )
  ...
  def start_trigger(self):
    if self.me.jager_position not in [self.me.position, self.opponent.position]:
      if self.game.make_fork(2, self.me, "Swap places with Jager?", ["No", "Yes"]):
        old_pos = self.me.position
        self.me.move_directly([self.me.jager_position])
        if self.me.position == self.me.jager_position:
          self.me.jager_position = old_pos
...
class Feral(Style):
  ...
  def evaluation_bonus(self):
    return 0.3 if self.opponent.position == self.me.jager_position else -0.1
...
class Dual(Style):
  ...
  def evaluation_bonus(self):
    return 0.45 if self.opponent.position == self.me.jager_position else -0.15
...
class Overload(Base):
  ...
  def start_trigger(self):
    ...
    available_styles = sorted(list(available_styles), key=attrgetter("order"))
    # choose one, and add it to active styles
    prompt = "Select a style to overload:"
    options = [s.name for s in available_styles]
    choice = self.game.make_fork(len(available_styles), self.me, prompt, options)
...
class Whip(Style):
  ...
  def get_preferred_range(self):
    return (2 if self.me.mutating in self.me.discard[1] else 1) * self.preferred_range
...
class Snare(Base):
  ...
  def special_range_hit(self):
    return (
      self.me.trap_position is not None
      and abs(self.me.trap_position - self.opponent.position) <= 1
    )
...
class Dragonflare(Finisher):
  ...
  def evaluate_setup(self):
    return 1 if self.game.distance() in (3, 4, 5) and self.me.counters >= 2 else 0
...
class Dragonbreath(Style):
  ...
  def hit_trigger(self):
    if self.me.attack_range() == 3 and self.me.spend_counters("for +2 power", 2, True):
      self.me.add_triggered_power_bonus(2)
...
class Invocation(Style):
  ...
  def take_a_hit_trigger(self):
    if self.me.active_familiar and self.game.make_fork(
      2, self.me, "Banish %s for Soak 2?" % self.me.active_familiar, ["No", "Yes"]
    ):
      if self.game.reporting:
        self.game.report(
          "Lesandra banishes her %s for Soak 2" % self.me.active_familiar
        )
...
class Wyvern(Familiar):
  ...
  def can_be_hit(self):
    return not (self is self.me.active_familiar and self.game.distance() == 4)
...
class Eternal(Style):
  ...
  def take_a_hit_trigger(self):
    if self.me.can_spend(1):
      self.me.eternal_soak = self.game.make_fork(
        1 + len(self.me.pool), self.me, "Spend how many tokens (+1 Soak each)?"
      )
      for unused in range(self.me.eternal_soak):
        self.me.spend_token()
      if self.me.eternal_soak and self.game.reporting:
        self.game.report(
          "Luc gains +%d soak (%d total)"
          % (self.me.eternal_soak, self.me.eternal_soak + 1)
        )
...
class Fathomless(Style):
  ...
  def evaluation_bonus(self):
    ...
    return (
      -0.5
      if self.opponent.dash in (self.opponent.discard[1] | self.opponent.discard[2])
      else 0.5
    )
...
class SolarSoul(Finisher):
  ...
  def hit_trigger(self):
    spend = self.game.make_fork(
      1 + self.me.trance, self.me, "Spend how many Trance counters (+1 Power each)?"
    )
    if spend:
      if self.game.reporting:
  ...
  def evaluate_setup(self):
    return 1 + 0.25 * self.me.trance if self.game.distance() <= 2 else 0
...
class AstralCannon(Finisher):
  ...
  def evaluate_setup(self):
    return 1 if self.me.counters >= 3 and self.game.distance() in (2, 3, 4) else 0
...
class Meditation(Base):
  ...
  def start_trigger(self):
    ...
    if self.game.reporting and self.me.meditation_soak:
      self.game.report("Marmelee gains Soak %d" % self.me.meditation_soak)
...
class Sorceress(Style):
  ...
  def before_trigger(self):
    if self.me.is_attacking():
      self.me.sorceress_range = self.me.spend_counters("for +0-2 range", 1, True)
...
class Nullifying(Style):
  ...
  def evaluation_bonus(self):
    return (-0.3 if self.me.position in (0, 6) else 0.15) + 0.05 * (
      self.me.counters - 2
    )
...
class MagnusMalleus(Finisher):
  ...
  def hit_trigger(self):
    if self.me.can_spend(1):
      spend = self.game.make_fork(
        1 + len(self.me.pool), self.me, "Spend how many tokens (+3 Power each)?"
      )
      for unused in range(spend):
        self.me.spend_token()
  ...
  def evaluate_setup(self):
    return 0.5 * len(self.me.pool) if self.me.attack_range() in [2, 3, 4] else 0
...
class Celestial(Style):
  ...
  def after_trigger(self):
    if self.me.life > 1:
      life_loss = self.game.make_fork(
        min(self.me.life, 4), self.me, "How much life would you like to lose?"
      )
...
class Unstable(Style):
  ...
  def hit_trigger(self):
    ...
    if n_effects:
      ...
      if self.opponent.is_user and self.game.interactive_mode:
        options = [
          ", ".join([self.effects_names[e] for e in combo]) for combo in combos
        ]
      result = self.game.make_fork(
        len(combos), self.opponent, "Choose effect/s for Oriana to activate:", options
      )
  ...
  def choose_discard(self):
    ...
    chosen_combo = combos[self.game.make_fork(len(combos), self.me, prompt, options)]
  ...
  effects_names = {
    choose_regain: "Oriana regains 5 MP",
...
class Calamity(Style):
  ...
  def hit_trigger(self):
    ...
    if mp >= 2:
      if self.opponent.pool and self.game.make_fork(
        2, self.opponent, "Discard a token to prevent loss of 2 life?", ["No", "Yes"]
      ):
        self.opponent.discard_token()
      else:
        self.opponent.lose_life(2)
...
class Gunner(Style):
  ...
  def before_trigger(self):
    if self.me.can_spend(1):
      if self.game.make_fork(
        2, self.me, "Discard a token for -1/+1 range?", ["No", "Yes"]
      ):
...
class Crossfire(Style):
  ...
  def hit_trigger(self):
    if self.me.can_spend(1):
      if self.game.make_fork(
        2, self.me, "Discard a token for +2 power?", ["No", "Yes"]
      ):
        self.me.spend_token()
        self.me.add_triggered_power_bonus(2)
...
class ArtificeAvarice(Finisher):
  ...
  def start_trigger(self):
    ...
    if self.game.reporting:
      self.game.report("Runika overcharges her " + self.me.overcharged_artifact.name)
...
class Explosive(Style):
  ...
  def hit_trigger(self):
    if self.me.active_artifacts:
      if self.game.make_fork(
        2, self.me, "Deactivate artifact to gain +2 power?", ["No", "Yes"]
      ):
...
class Field(Base):
  ...
  def special_range_hit(self):
    return (
      self.me.projection is not None
      and abs(self.opponent.position - self.me.projection) == 1
    )
...
class FortuneBuster(Finisher):
  ...
  def evaluate_setup(self):
    return (
      0.5 if self.opponent.special_action_available and self.opponent.life <= 7 else 0
    )
...
class Wyrding(Style):
  ...
  def start_trigger(self):
    if self.me.correct_guess:
      ...
      available_bases = sorted(list(available_bases), key=attrgetter("order"))
      prompt = "Choose new base:"
      options = [b.name for b in available_bases] + ["Keep my base"]
      ans = self.game.make_fork(len(available_bases) + 1, self.me, prompt, options)
...
class Combination(Style):
  ...
  def evaluation_bonus(self):
    return (
      (0.4 if self.game.distance() <= 2 else -0.4)
...
class Spiral(Style):
  ...
  def before_trigger(self):
    self.me.advance(
      [0, 1, 2, 3], specific_movement_reaction=self.specific_movement_reaction
    )
...
class CurtainCall(Finisher):
  ...
  def before_trigger(self):
    for puppet in self.me.puppets:
      if puppet.position is not None and abs(puppet.position - self.me.position) == 1:
  ...
  def hit_trigger(self):
    for puppet in self.me.puppets:
      if puppet is not self.me.possessed_puppet:
        ...
        pos = positions[self.game.make_fork(len(positions), self.me, prompt, options)]
        puppet.position = pos
        if old_pos == self.opponent.position and pos != old_pos:
          self.me.add_triggered_power_bonus(1)
    if self.game.reporting:
...
class Climactic(Style):
  ...
  @property
  def ordered_before_trigger(self):
    return self.me.mephisto.position not in [
      self.me.position,
      self.opponent.position,
      None,
    ]
...
class Storyteller(Style):
  ...
  def get_preferred_range(self):
    ...
    if (
      self.me.position == self.me.eris.position
      and self.me.possessed_puppet is not self.me.eris
    ):
      return 1.5
    else:
      return 0
  ...
  def can_be_hit(self):
    return not (
      self.me.possessed_puppet is self.me.loki and self.opponent.attack_range() == 1
    )
  ...
  def evaluation_bonus(self):
    loki = self.me.loki
    possible_puppets = [p for p in self.me.puppets if p.position == self.me.position]
    possible_values = []
    if loki in possible_puppets:
      possible_values.append(
        0.5 if abs(loki.position - self.opponent.position) == 1 else -0.1
      )
...
class Distressed(Style):
  ...
  def get_preferred_range(self):
    ...
    if me == eris and not me in (self.me.loki.position, self.me.mephisto.position):
      return 0
  ...
  def end_trigger(self):
    puppets = [p for p in self.me.puppets if p is not self.me.possessed_puppet]
    ...
    if self.me.is_user and self.game.interactive_mode:
      for pos in positions:
        ...
        opt[pos] = puppet.initial
        options.append("".join(opt))
    pos = positions[self.game.make_fork(7, self.me, prompt, options)]
    puppet.position = pos
    if self.game.reporting:
  ...
  def evaluation_bonus(self):
    me = self.me.position
    if me == self.me.eris.position and not me in (
...
class TsunamisCollide(Finisher):
  ...
  def can_hit(self):
    return (
      self.me.juto_position is not None
      and abs(self.opponent.position - self.me.juto_position) > 1
    )

  def reveal_trigger(self):
    tsunami_spaces = (
      abs(self.me.position - self.me.juto_position) - 1 if self.me.zone_3() else 0
    )
...
class Empathic(Style):
  ...
  def start_trigger(self):
    if self.me.juto_position not in [None, self.me.position, self.opponent.position]:
      if self.game.make_fork(2, self.me, "Swap positions with Juto?", ["No", "Yes"]):
        old_pos = self.me.position
...
class Judgment(Style):
  ...
  def blocks_opponent_movement(self, initiator, direct):
    ...
    return set([self.me.position, self.opponent.position - direction])
...
class Paladin(Style):
  ...
  def end_trigger(self):
    self.me.move_directly((self.opponent.position + 1, self.opponent.position - 1))
...
class Monster(Style):
  ...
  def before_trigger(self):
    ...
    self.me.advance([1])
    while (
      self.me.position in self.me.zombies
      and self.me.position not in visited_positions
      and self.game.make_fork(2, self.me, "Keep advancing with Monster?", ["No", "Yes"])
    ):
...
class OpenTheGate(Finisher):
  ...
  def evaluate_setup(self):
    ...
    return (
      2
      if dist <= 2
      or (dist == 3 and self.me.fluidity in self.me.paradigms)
      or (dist in (3, 4) and self.me.distortion in self.me.paradigms)
      else 0
    )
...
class PlaneDivider(Finisher):
  ...
  def hit_trigger(self):
    ...
    paradigm = self.me.paradigms[self.game.make_fork(5, self.me, prompt, options)]
